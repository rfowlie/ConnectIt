...
|---|---|
| [UnrealGridMechanics](../../../Plugins/UnrealGridMechanics/Docs/README.md) | The 2D grid/board framework — tiles, pieces, board-state replication + interpreter pattern, shift system, grid math. The foundation ConnectIt's board is built on. |
...
- [Duplication.md](Duplication.md) — eight ranked entries cataloguing classes/subsystems that overlap or duplicate each other's responsibilities, from the entire legacy pipeline down to a couple of small commented-out code blocks — each with evidence, a hypothesis for why it happened, and a recommendation.

## Known Issues

Three confirmed correctness bugs, surfaced here rather than only inside `Duplication.md`, because they're defects, not structural overlap, and are worth a new contributor's attention immediately:
...
- **`ConnectIt_TileStateInterpreter::FindTileActor`** (`Source/ConnectIt/Private/Interpreter/ConnectIt_TileStateInterpreter.cpp:108`) — unconditionally returns `nullptr`, a leftover from a removed tile-lookup mapping that was never replaced. Makes `SendTagToTile` a permanent no-op: `ProcessTileChange` still correctly detects tile state changes, but no `ConnectIt.Tile.*` visual-feedback tag ever actually reaches a tile actor through this path. Fix already identified: route through `UGridTileRegistryComponent::GetTileAtPosition(FGridPosition)`, the same lookup every other position↔tile resolution in the project already uses (exposed via `AConnectIt_BoardManager::GetTileRegistry()`). See [Duplication.md #7](Duplication.md#7-two-near-identical-permanent-no-ops-with-the-same-known-fix) for the related interpreter with the same root cause.
