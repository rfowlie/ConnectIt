...
|---|---|---|---|---|
| 1 | "Chimera" naming residue | `UnrealCodingUtils::IsAuthoritative` — `Category = "Chimera|Utils"` ([`CodingUtilsComponentLibrary.h:21`](../../../Plugins/UnrealCodingUtils/Source/UnrealCodingUtils/Public/Library/CodingUtilsComponentLibrary.h)); `UnrealGridMechanics::UBoardStateInterpreter` — `ClassGroup=(Chimera)` and doc comment *"Lives in Chimera Framework -- no project-specific type knowledge"* ([`BoardStateInterpreter.h:12,15`](../../../Plugins/UnrealGridMechanics/Source/UnrealGridMechanics/Public/Board/Interpreter/BoardStateInterpreter.h)) | Two metadata strings reference "Chimera," a project name that predates "ConnectIt." Neither is user-facing (Blueprint editor category/class-group only), but both are stale identifiers from before the plugin suite was generalized/renamed. | Copy-paste carried over from an earlier codebase (likely an earlier, differently-named project this plugin suite originated in) during extraction into standalone, project-agnostic plugins; the rename sweep missed these two spots. |
