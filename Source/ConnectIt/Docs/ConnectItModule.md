...
| Class | Base | Purpose |
|---|---|---|
| `AConnectIt_BoardManager` | `AActor`, `Blueprintable` | The board orchestrator. Owns/creates every board-related component as default subobjects (`TileRegistryComponent`, `PieceRegistryComponent`, `ConnectItConfigComponent`, `BoardStateComponent`, `BoardShiftComponent`, `BoardRulesComponent`, `BoardSequencerComponent`, plus interpreters). Public API: `ProcessRequest(FTurnActionRequest)` — the single entry point for all board mutations, dispatched by `RequestType` tag to private, server-only, `HasAuthority()`-gated `HandlePlacePieceRequest`/`HandleShiftRequest`; `InitialiseBoard(int32 NumFactions)`. Drives turn-end resolution holding via `BindParticipantManager`/`HandleTurnResolutionStarted` (see [TurnResolutionHold.md](Workflows/TurnResolutionHold.md)). Does **not** maintain its own `OnPiecePlaced`/`OnLineScored`/etc. delegates — those were removed in favor of gameplay tags (see [GameplayTag-EventSequencing.md](Workflows/GameplayTag-EventSequencing.md)). |
