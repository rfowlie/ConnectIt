...
| Class | Base | Purpose |
|---|---|---|
| `AConnectIt_BoardManager` | `AActor`, `Blueprintable` | The board orchestrator. Owns/creates every board-related component as default subobjects (`TileRegistryComponent`, `PieceRegistryComponent`, `ConnectItConfigComponent`, `BoardStateComponent`, `BoardShiftComponent`, `BoardRulesComponent`, `BoardSequencerComponent`, plus interpreters). Public API: `ProcessRequest(FTurnActionRequest)` — the single entry point for all board mutations, dispatched by `RequestType` tag to private, server-only, `HasAuthority()`-gated `HandlePlacePieceRequest`/`HandleShiftRequest`; `InitialiseBoard(int32 NumFactions)`. Registers a persistent `UGameEventTask_Async` against `ConnectIt_Event_TurnEnd` in `BindParticipantManager` (`RegisterTurnEndTask`/`HandleTurnEndTaskExecute`) so `UTurnBasedParticipantManagerComponent::EndTurn` — which now triggers that tag directly — waits for board-change visuals to finish before advancing (see [GameplayTag-EventSequencing.md](Workflows/GameplayTag-EventSequencing.md)'s "TURN SYSTEM NOW WAITS ON THIS" section). Does **not** maintain its own `OnPiecePlaced`/`OnLineScored`/etc. delegates — those were removed in favor of gameplay tags. |
| `UConnectIt_BoardStateComponent` | `UBoardStateComponentBase` (`UnrealGridMechanics`) | **The single source of truth for board state**, and the only replicated data-carrying component in the module. One property: `UPROPERTY(ReplicatedUsing=OnRep_BoardSnapshot) FConnectItBoardStateSnapshot BoardSnapshot`. Server API: `InitialiseBoardState()`, `SetBoardState(NewState, ChangeEvent)` (captures previous, applies new, fires `OnBoardStateChanged` immediately on server; `check(IsAuthoritative())`-guarded). Read API: `GetCurrentState()`, `GetPreviousState()`, `GetChangeEvent()` (`BlueprintPure`). Clients receive the same broadcast via `OnRep_BoardSnapshot`. |
...
| `UConnectIt_BoardSequencerComponent` | `UActorComponent` | Drives the gated visual sequence for board changes (shift, or PiecePlaced→LineScored→PlayerWin in order) via `UGameEventTaskSubsystem::QueueTagSequence`. Self-contained: binds to the sibling `BoardStateComponent`'s change signal in `BeginPlay`. Exposes `IsIdle()`/`IsSequenceInFlight()`/`OnSequenceIdle` as pure board-domain facts, with zero knowledge of turns. See [GameplayTag-EventSequencing.md](Workflows/GameplayTag-EventSequencing.md). |
