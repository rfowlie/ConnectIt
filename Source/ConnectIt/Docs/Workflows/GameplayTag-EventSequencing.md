...
| Project-specific tag vocabulary | `ConnectIt.Event.PiecePlaced/LineScored/PlayerWin/TurnEnd` (`Source/ConnectIt/ConnectIt_GameplayTags.h`) | The tags themselves are ConnectIt-specific; the subsystem that keys off them is not. (`ConnectIt.Event.Shift` was removed along with the rest of the dead shift pipeline — see [ConnectItModule.md](../ConnectItModule.md#board-architecture-overhaul).) |
...
2. The same component that just committed the change (`UConnectIt_BoardStateComponent` in ConnectIt) inspects what changed and calls the enqueue primitive once per event, in order — e.g. a placement always leads with `PiecePlaced`, then conditionally `LineScored` and/or `PlayerWin`.
