...
| Listeners (both sides, identical code) | `UConnectIt_BoardSequencerComponent`, `UConnectIt_TileStateInterpreter`, `UConnectIt_PieceSpawnInterpreter`, `UConnectIt_ScoreInterpreter` (all bind `OnBoardStateChanged`) | None of these know or care whether they're running because of a local `SetBoardState` call or a replication update. |
