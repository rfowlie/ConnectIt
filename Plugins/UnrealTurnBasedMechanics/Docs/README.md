...
- `UDWidget_TurnBasedParticipantManagerComponent::BindDelegates` resolves its tracked source via `GetOwningPlayer()->FindComponentByClass<UTurnBasedParticipantManagerComponent>()`, but that component is a subobject of `ATurnBasedGameState`, not the player controller -- `FindComponentByClass` only searches the calling actor's own components, so this call likely always returns null and the widget never resolves. Not previously documented. See [Systems.md](Systems.md#known-rough-edges).
