...
A `_decisions/` folder holds **one dated note per non-obvious decision**, so a settled
...
- Optional: a `_decisions/__INDEX.md` (per [[TIndex]]) may carry a dated table of every
  decision for quick scanning.

## Deviations

Add `_decisions/__README.md` starting `Extends _schema/_decisions.md. Differences:` if a
