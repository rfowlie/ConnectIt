...
| Grid position ↔ world position, rows/columns, board dimensions | `UGridTileRegistryBase::GridPositionToWorld/WorldToGridPosition/GetRow/GetColumn/GetMinRow.../GetRowCount/GetColumnCount` | `UConnectIt_GameUtilityLibrary::GetTileRegistry()` → `UConnectIt_BoardRegistrySubsystem::GetTileRegistry()` (one canonical per-world instance, resolved identically on server and every client) | Pure — **can still return null**: the subsystem duplicates its runtime instance from an `EditAnywhere, Instanced` template on the level's `ConnectIt_LevelConfigDataAsset` at `OnWorldBeginPlay`; if that template is unset for the level, or `OnWorldBeginPlay` hasn't fired yet, this is null. Check validity before use. |
| Piece actor at a grid position | `UGridPieceRegistryBase::GetPiece(Position)` | `UConnectIt_GameUtilityLibrary::GetPieceRegistry()` → `UConnectIt_BoardRegistrySubsystem::GetPieceRegistry()` | Pure — same null-unless-templated caveat as above; not every level needs a `PieceRegistry` template authored yet (`InitialiseRegistry()` is still a no-op stub) |
...
getters (`GetTileRegistry()`/`GetPieceRegistry()`, now on
`UConnectIt_BoardRegistrySubsystem`), `GetLevelConfig()`, `GetPiece()`. None of the checked components expose
