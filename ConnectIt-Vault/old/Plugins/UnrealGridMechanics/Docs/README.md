...
- Needing a generic, replicated board-state container with a broadcast point any listener can bind to directly (`UBoardStateComponentBase`).
...
- Plain broadcast for board state: `UBoardStateComponentBase::OnBoardStateChanged` is a public multicast delegate any listener (Blueprint or C++) binds to directly -- no registry/observer-object layer in between. See [Systems.md](Systems.md#board-state) for how the replication side actually works end to end.
